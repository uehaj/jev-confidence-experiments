...
def run(name, state, q):
    ...
    out[name]={"model":d.get("model"),"team":a["team"]["choice"],
      ...
      "team_probs":{k:round(v,2) for k,v in a["team"]["probabilities"].items()},
    ...
    o=out[name]
    print(f"{name:26} {o['team']:12} conf={o['team_conf']:.2f}  urg={o['urgency']:.2f}/{o['urgency_conf']:.2f}  "
          f"repro={o['has_repro']:.2f} enough={o['enough_info']:.2f}",flush=True)

for k in S_JA: run(f"JA/{k}", S_JA[k], Q_JA)
for k in S_EN: run(f"EN/{k}", S_EN[k], Q_EN)
json.dump(out,open("jev_results_lang.json","w"),indent=1,ensure_ascii=False)
